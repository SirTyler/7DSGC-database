    ...
    getDescription(rankIndex: number) {
        if(rankIndex > 0) {
            return (
                ...
                {(theme) => (
                    <>
                        Inflicts damage equal to <span className={`damage ${theme.theme}`}> {this.ranks[rankIndex].modifier}% </span> of Attack on all enemies and {this.ranks[rankIndex].effect[0]} for <span className={`subeffect ${theme.theme}`}> {this.ranks[rankIndex].extra[0]} turn(s)</span>.
                    </>
                )}
                </ThemeContext.Consumer>
            )
